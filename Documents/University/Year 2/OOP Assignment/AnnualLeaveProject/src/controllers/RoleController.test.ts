...
describe('RoleController', () => {
    ...
    it('getById returns a role if a valid id is supplied', async () => {
        const validManagerDetails = getValidManagerData();
        const req = mockRequest({ id: validManagerDetails.roleId });
        const res = mockResponse();
        ...
        expect(mockRoleRepository.findOne).toHaveBeenCalledWith({ where: { id: validManagerDetails.roleId } });
        expect(ResponseHandler.sendSuccessResponse).toHaveBeenCalledWith(res, 
                                                                        validManagerDetails);
    });
    ...
    it('delete will return SUCCESS if the role is successfully deleted', async () => {
        ...
        expect(mockRoleRepository.delete).toHaveBeenCalledWith(validManagerDetails.roleId);
    ...
    it('update returns a BAD_REQUEST if id provided does not exist', async () => {
        const validManagerDetails = getValidManagerData();
        const req = mockRequest({}, {id: validManagerDetails.roleId, name: validManagerDetails.name}); 
        const res = mockResponse();
        ...
        expect(mockRoleRepository.findOneBy).toHaveBeenCalledWith({id: validManagerDetails.roleId});
        expect(ResponseHandler.sendErrorResponse).toHaveBeenCalledWith(res, 
                                                                        StatusCodes.BAD_REQUEST, 
    ...
    it('update will return a BAD_REQUEST if the name does not exist/blank', async () => {
        const validManagerDetails = getValidManagerData();
        const req = mockRequest({},  {id: validManagerDetails.roleId, name: BLANK_ROLE_NAME});
        const res = mockResponse();
    ...
    it('update with valid details', async () => {
        ...
        roleDetailsToChange.roleId = validManagerDetails.roleId;
        ...
        expect(mockRoleRepository.findOneBy).toHaveBeenCalledWith({id: validManagerDetails.roleId});
        expect(mockRoleRepository.save).toHaveBeenCalledWith( {id: validManagerDetails.roleId, name: validManagerDetails.name} );
        expect(ResponseHandler.sendSuccessResponse).toHaveBeenCalledWith(res, 
                                                                        validManagerDetails);
    });
});